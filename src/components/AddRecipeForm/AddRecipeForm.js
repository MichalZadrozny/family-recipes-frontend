...
class AddRecipeForm extends Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (selectedRecipe != null) {
      history.push(`/recipe/${selectedRecipe.id}`);
    }
  }
    ...
    return (
      <Formik
        ...
        onSubmit={async (values, { setSubmitting }) => {
          ...
          tempValues.ingredients = removeIDs(tempValues.ingredients);
          tempValues.steps = mapStepsToStringArray(tempValues.steps);

          addRecipe(tempValues);
...
const mapStateToProps = (state) => {
  const { adding, selectedRecipe } = state.recipes;
  return { adding, selectedRecipe };
};
...
export default compose(withRouter, connect(mapStateToProps, mapDispatchToProps))(AddRecipeForm);
