export default {
  GET_PREVIEWS_REQUEST: 'GET_PREVIEWS_REQUEST',
  GET_PREVIEWS_SUCCESS: 'GET_PREVIEWS_SUCCESS',
  GET_PREVIEWS_FAILURE: 'GET_PREVIEWS_FAILURE',

  GET_USER_PREVIEWS_REQUEST: 'GET_USER_PREVIEWS_REQUEST',
  GET_USER_PREVIEWS_SUCCESS: 'GET_USER_PREVIEWS_SUCCESS',
  GET_USER_PREVIEWS_FAILURE: 'GET_USER_PREVIEWS_FAILURE',

  ADD_RECIPE_REQUEST: 'ADD_RECIPE_REQUEST',
  ADD_RECIPE_SUCCESS: 'ADD_RECIPE_SUCCESS',
  ADD_RECIPE_FAILURE: 'ADD_RECIPE_FAILURE',

  GET_RECIPE_REQUEST: 'GET_RECIPE_REQUEST',
  GET_RECIPE_SUCCESS: 'GET_RECIPE_SUCCESS',
  GET_RECIPE_FAILURE: 'GET_RECIPE_FAILURE',

  ADD_RATING_REQUEST: 'ADD_RATING_REQUEST',
  ADD_RATING_SUCCESS: 'ADD_RATING_SUCCESS',
  ADD_RATING_FAILURE: 'ADD_RATING_FAILURE',
};